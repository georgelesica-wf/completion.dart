...
String generateCompletionScript(List<String> arguments) {
  final binNames = new List<String>();
  final scriptLines = new List<String>();
  ...
  scriptLines.add(prefix);

  // empty line
  scriptLines.add(prefix);

  for (final binName in binNames) {
    scriptLines.add(_getBinName(binName));
  }
  ...
  scriptLines.addAll(detailLines.map((l) => '## $l'));
...
String _getBinName(String binName) {
